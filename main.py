...
# Add globals to fix PyTorch >= 2.6 load
add_safe_globals([XttsConfig, XttsAudioConfig, XttsArgs, BaseDatasetConfig])

app = FastAPI(
    title="3VO Voice Clone API",
    description="Clone your voice and generate audio from text with XTTS",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allow any frontend, you can restrict this later
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Lazy load model (avoids delay on boot)
tts = None
...
@app.post("/speak_caption")
async def speak_caption(user_id: str = Form(...), text: str = Form(...)):
    global tts
    voice_path = f"./voices/{user_id}.wav"
    if not os.path.exists(voice_path):
        return {"error": "Voice sample not found."}
